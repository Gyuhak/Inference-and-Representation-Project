...
class Train_occ(Base):
    ...
    def train_main(self):
        ...
        x = tf.placeholder(tf.float32, shape=[None, 131072])
        y_ = tf.placeholder(tf.float32, shape=[None, 65536])

        x_img = tf.reshape(x, [self.batch_size, 256, 256, 2])
        ...
        # CL1 (256 -> 128)
        h_conv1 = self.conv(x_img, 2, 32, strides=[1, 2, 2, 1])
        print('h_conv1: ', h_conv1)

        # CL2 (128 -> 64)
        h_conv2 = self.conv(h_conv1, 32, 64, strides=[1, 2, 2, 1])
        print('h_conv2: ', h_conv2)

        # CL3 (64 -> 32)
        h_conv3 = self.conv(h_conv2, 64, 128, strides=[1, 2, 2, 1])
        print('h_conv3: ', h_conv3)

        # CL4 (32 -> 16)
        h_conv4 = self.conv(h_conv3, 128, 256, strides=[1, 2, 2, 1])
        print('h_conv4: ', h_conv4)

        print('-- deconvolutions ---')

        h_dconv4 = self.deconv(h_conv4, 256, 128,
            [self.batch_size, 32, 32, 128], strides=[1, 2, 2, 1])
        print('h_dconv4:', h_dconv4)

        h_dconv3 = self.deconv(h_dconv4, 128, 64,
            [self.batch_size, 64, 64, 64], strides=[1, 2, 2, 1])
        print('h_dconv3:', h_dconv3)

        h_dconv2 = self.deconv(h_dconv3, 64, 32,
            [self.batch_size, 128, 128, 32], strides=[1, 2, 2, 1])
        print('h_dconv2: ', h_dconv2)

        h_dconv1 = self.deconv(h_dconv2, 32, 1,
            [self.batch_size, 256, 256, 1], strides=[1, 2, 2, 1])
        print('h_dconv1: ', h_dconv1)

        h_dconv1_flat = tf.reshape(h_dconv1, [self.batch_size, 65536])

        cost = tf.reduce_mean(tf.square(y_ - h_dconv1_flat))
        # cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(h_dconv1_flat, y_))
        train_step = tf.train.AdamOptimizer(1e-3).minimize(cost)
        accuracy = tf.reduce_mean(tf.square(y_ - h_dconv1_flat))
        # accuracy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(h_dconv1_flat, y_))
        ...
        for i in range(300):
            ...
            if i % 2 == 0:
                ...
                train_accuracy, img, gt = sess.run([accuracy, h_dconv1, y_img],
                                                   feed_dict={x: batch[0], y_: batch[1]})
                print("step %d, training accuracy: %g" % (i, train_accuracy))

                fig = self.init_figure(figsize=(10, 5))
                rn, cn = 1, 2
                utils.imshow_in_subplot(rn, cn, 1, img[0, :, :, 0])
                utils.imshow_in_subplot(rn, cn, 2, gt[0, :, :, 0])
